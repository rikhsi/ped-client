...
import { ApplicationsApiService, SeasonApiService } from '@api/controllers';
import {
  EduDirection,
  DistrictShortItem,
...
@Injectable()
export class AttestationInstitutionFormService {
  ...
  readonly showAlertEducation = signal<boolean>(false);

  readonly form = new FormGroup<InstitutionForm>({
  ...
  public initForm(
    ...
    const { institutionId } = mainForm.getRawValue();
  ...
  public initDirectionListener(
    seasonId: number,
  ): Observable<InstitutionShortItem[]> {
    const { institutionId, districtId, eduDirection } = this.form.controls;

    return eduDirection.valueChanges.pipe(
      distinctUntilChanged(),
      tap(() => {
        this.showAlertEducation.set(false);
      ...
      switchMap((directionId) => {
        const pinpp = this.authService.user()?.pinpp;

        return this.applicationApi
          .checkPositionApplication$(pinpp, directionId)
          .pipe(
            tap(({ result }) => {
              this.showAlertEducation.set(!result);
